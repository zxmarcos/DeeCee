namespace DeeCee.SH4.Test
{
    ...
    public unsafe class ControlOpsTest
    {
        ...
        public void TestSrOps() // Status Register
        {
            ...
            ExecuteAndAssert(() => _state.R[regNum] = 0xFFFFFFFF, (s, m) => Assert.That(s.SR, Is.EqualTo(0xFFFFFFFF & MASK_SR)));
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0xAAAAAAAA); }, (s, m) => {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.SR = 0xBBBBBBBB; }, (s, m) => {
        ...
        public void TestGbrOps() // Global Base Register
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0x11223344); }, (s, m) => {
                Assert.That(s.GBR, Is.EqualTo(0x11223344));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });
            
            CompileInstruction(Sh4Assembler.STCMGBR(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.GBR = 0x55667788; }, (s, m) => {
        ...
        public void TestVbrOps() // Vector Base Register
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0x12345678); }, (s, m) => {
                Assert.That(s.VBR, Is.EqualTo(0x12345678));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });
            
            CompileInstruction(Sh4Assembler.STCMVBR(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.VBR = 0x87654321; }, (s, m) => {
        ...
        public void TestSsrOps() // Saved Status Register
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0xCCCC); }, (s, m) => {
                Assert.That(s.SSR, Is.EqualTo(0xCCCC));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });

            CompileInstruction(Sh4Assembler.STCMSSR(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.SSR = 0xDDDD; }, (s, m) => {
        ...
        public void TestSpcOps() // Saved Program Counter
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0x1010); }, (s, m) => {
                Assert.That(s.SPC, Is.EqualTo(0x1010));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });

            CompileInstruction(Sh4Assembler.STCMSPC(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.SPC = 0x2020; }, (s, m) => {
        ...
        public void TestDbrOps() // Debug Base Register
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0x9090); }, (s, m) => {
                Assert.That(s.DBR, Is.EqualTo(0x9090));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });

            CompileInstruction(Sh4Assembler.STCMDBR(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.DBR = 0xA1A1; }, (s, m) => {
        ...
        public void TestMacOps() // MACH and MACL Registers
        {
            ...
            ExecuteAndAssert(() => _state.R[regNumH] = 0xAAAAAAAA, (s, m) => Assert.That(s.MACH, Is.EqualTo(0xAAAAAAAA)));
            CompileInstruction(Sh4Assembler.STSMACH(regNumH));
            ExecuteAndAssert(() => _state.MACH = 0xBBBBBBBB, (s, m) => Assert.That(s.R[regNumH], Is.EqualTo(0xBBBBBBBB)));

            CompileInstruction(Sh4Assembler.LDSMACL(regNumL));
            ExecuteAndAssert(() => _state.R[regNumL] = 0xCCCCCCCC, (s, m) => Assert.That(s.MACL, Is.EqualTo(0xCCCCCCCC)));
            CompileInstruction(Sh4Assembler.STSMACL(regNumL));
            ExecuteAndAssert(() => _state.MACL = 0xDDDDDDDD, (s, m) => Assert.That(s.R[regNumL], Is.EqualTo(0xDDDDDDDD)));
            
            CompileInstruction(Sh4Assembler.LDSMMACH(regNumH));
            ExecuteAndAssert(() => { _state.R[regNumH] = memAddrH; _memory.Write32(memAddrH, 0x1234); }, (s, m) => {
            ...
            ExecuteAndAssert(() => { _state.R[regNumL] = memAddrL + 4; _state.MACL = 0x5678; }, (s, m) => {
        ...
        public void TestPrOps() // Procedure Register
        {
            ...
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr; _memory.Write32(memAddr, 0xABCDE); }, (s, m) => {
                Assert.That(s.PR, Is.EqualTo(0xABCDE));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr + 4));
            });

            CompileInstruction(Sh4Assembler.STSMPR(regNum));
            ExecuteAndAssert(() => { _state.R[regNum] = memAddr + 4; _state.PR = 0xFEDCB; }, (s, m) => {
                Assert.That(m.Read32(memAddr), Is.EqualTo(0xFEDCB));
                Assert.That(s.R[regNum], Is.EqualTo(memAddr));
            });
        }
        
        // Os testes para R_BANK (LDC/STC R_BANK) não foram adicionados
        // pois a funcionalidade não está implementada na classe ControlOps fornecida.
    }
}